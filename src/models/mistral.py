...
class Mistral():
	def __init__(self, path:Path, num_labels:int, model_name:str, batch_size:int, epochs:int, seed:int, all_toks:int):
	...
	def train_model(self, model, tokenizer, train_dataset, valid_dataset):
		...
		checkpoint_output_path = CHECKPOINTS_ROOT / self.model_name.split("/")[-1]
		...
		trainer = Trainer(
			...
			compute_metrics=build_compute_metrics(valid_dataset, self.all_toks),
	...
	def run(self):
		...
		trainer = Mistral.train_model(self, model, tokenizer, train_dataset, valid_dataset)
